...
import {
  ...
  buildPageComponentOptions,
  buildReorderItems,
  normalizeParentId,
  persistExpanded,
  readExpandedFromStorage as readExpandedStateFromStorage,
  sanitizeExpanded,
  toRouteName,
  toSlug,
  treeFilter,
  type MenuNode,
  type NodeType,
} from '@/views/system/menu/helpers';
import { useMenuDialog } from '@/views/system/menu/hooks/useMenuDialog';
import { useMenuForm } from '@/views/system/menu/hooks/useMenuForm';
import { useMenuTable } from '@/views/system/menu/hooks/useMenuTable';
import type { OpenType, RoleRow } from '@/views/system/menu/schema';
...
const { keyword, loading, savingOrder, data, expandedTreeNodes, expandAll } = useMenuTable();
...
const loadExpandedFromStorage = () => {
  return readExpandedStateFromStorage(() =>
...
const reload = async () => {
  ...
  try {
    ...
    const storedExpanded = loadExpandedFromStorage();
...
const { drawerVisible } = useMenuDialog();
const drawerTitle = computed(() => (mode.value === 'create' ? '添加菜单' : '编辑菜单'));
const { formRef, form } = useMenuForm();
