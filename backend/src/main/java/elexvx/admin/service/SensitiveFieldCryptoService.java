...
public class SensitiveFieldCryptoService {
  ...
  public SensitiveFieldCryptoService(ElexvxCoreProperties properties) {
    String secret = properties.getSecurity().getFieldSecret();
    String effective = secret == null ? "" : secret.trim();
    if (!StringUtils.hasText(effective)) {
      throw new IllegalStateException("elexvx.security.field-secret 未配置，服务拒绝启动");
    }
    if (effective.getBytes(StandardCharsets.UTF_8).length < 32) {
      throw new IllegalStateException("elexvx.security.field-secret 强度不足，要求至少 32 bytes");
    }
