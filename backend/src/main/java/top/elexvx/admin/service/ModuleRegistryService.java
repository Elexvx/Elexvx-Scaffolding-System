...
import top.elexvx.admin.exception.ModuleUnavailableException;
import top.elexvx.admin.mapper.ModuleRegistryMapper;
import top.elexvx.admin.module.ModuleDefinition;
import top.elexvx.admin.module.ModuleDefinitionRegistry;
import top.elexvx.admin.module.ModuleInstallationContext;
import top.elexvx.admin.vo.ModuleDescriptor;
import top.elexvx.admin.vo.ModuleRegistryResponse;
...
public class ModuleRegistryService {
  ...
  private boolean isRuntimeDependencyAvailable(String key) {
    if ("sms".equals(key)) {
      if (!getFlag("elexvx.modules.sms.enabled", true)) return false;
      return isClassPresent("com.aliyuncs.DefaultAcsClient")
        || isClassPresent("com.tencentcloudapi.sms.v20210111.SmsClient");
    }
    if ("email".equals(key)) {
      if (!getFlag("elexvx.modules.email.enabled", true)) return false;
