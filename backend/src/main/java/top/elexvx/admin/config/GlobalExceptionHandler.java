...
import top.elexvx.admin.exception.ConcurrentLoginException;
import top.elexvx.admin.exception.ErrorCodes;
import top.elexvx.admin.exception.ModuleUnavailableException;
import top.elexvx.admin.exception.RepeatSubmitException;
import top.elexvx.admin.mapper.MenuItemMapper;
...
public class GlobalExceptionHandler {
  ...
  public ResponseEntity<ApiResponse<Void>> handleBusinessException(BusinessException e) {
    int code = e.getCode();
    String message = e.getMessage() == null || e.getMessage().isBlank() ? "请求处理失败" : e.getMessage();
    HttpStatus status = HttpStatus.resolve(code);
    if (status == null || status.is2xxSuccessful()) {
      status = HttpStatus.BAD_REQUEST;
    }
    if (status.is5xxServerError()) {
      log.error("业务异常(code={}): {}", code, message, e);
    } else {
