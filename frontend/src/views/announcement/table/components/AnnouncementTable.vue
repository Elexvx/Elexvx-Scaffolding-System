  ...
  <t-table :data="data" :columns="columns" row-key="id" :loading="loading" :pagination="pagination" @page-change="$emit('page-change', $event)">
    ...
    <template #op="{ row }">
      <t-space size="small">
        ...
        <t-popconfirm v-if="row.status === 'published'" content="确认撤回该公告？" @confirm="$emit('toggle-publish', row)">
        ...
        <t-popconfirm theme="danger" content="确认删除该公告？" @confirm="$emit('delete', row.id)">
