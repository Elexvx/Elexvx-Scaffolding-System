...
public class FileTokenService {
  ...
  public FileTokenService(
    ...
    @org.springframework.beans.factory.annotation.Value("${server.servlet.context-path:/api}") String contextPath
  ) {
    String secret = properties.getFile().getTokenSecret();
    long tokenTtlSeconds = properties.getFile().getTokenTtlSeconds();
    ...
    if (effective.isEmpty()) {
      ...
      log.warn("File token secret is empty; using ephemeral random key. "
        + "Set elexvx.file.token-secret to keep links stable across restarts.");
    } else {
